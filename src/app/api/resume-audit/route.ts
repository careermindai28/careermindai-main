...
function transformToUIFormat(openAIResponse: OpenAIResumeAuditResponse) {
  return {
    ...
    improvements: openAIResponse.improvements
      .slice(0, 5)
    ...
    atsRecommendations: openAIResponse.sectionFeedback
      .slice(0, 5)
      .map((section, index) => ({
  ...
  try {
    ...
    if (!body.resumeText || body.resumeText.trim().length < 100) {
      return NextResponse.json(
        {
          error:
            "Resume text is required and must be at least 100 characters long.",
        },
        { status: 400 }
      );
    }
...
Provide your analysis in the following JSON structure (return ONLY the JSON, no other text):
{
  ...
  "roleFitNotes": "<role-specific advice>"
    ...
    parsedResponse.subscores.atsScore = clampScore(
      parsedResponse.subscores.atsScore
    );
    parsedResponse.subscores.contentScore = clampScore(
      parsedResponse.subscores.contentScore
    );
    parsedResponse.subscores.formattingScore = clampScore(
      parsedResponse.subscores.formattingScore
    );
    parsedResponse.subscores.impactScore = clampScore(
      parsedResponse.subscores.impactScore
    );
