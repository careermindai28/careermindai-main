...
async function extractFromPdf(buffer: Buffer): Promise<string> {
  // Works with both ESM + CJS builds of pdf-parse
  const mod: any = await import("pdf-parse");
  const parse = mod?.default ?? mod; // default if present, otherwise module itself
...
// Optional: helps quickly verify route exists in production
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!ct.includes("multipart/form-data")) {
      return jsonError(
        "Resume audit expects file upload (multipart/form-data).",
        415,
